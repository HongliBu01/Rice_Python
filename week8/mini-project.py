...
class Sprite:
    ...
    def collide(self, other_project):
        if dist(self.pos, other_project.get_position()) <= (self.radius + other_project.get_radius()):
            return True
        else :
            return False
...
def draw(canvas):
    ...
    canvas.draw_text("Score : " + str(score), [680, 50], 26, "White")
...
def rock_spawner():
    ...
    if started:
        if len(rock_group) <= 12 and dist(rock_pos, my_ship.get_position()) > (asteroid_info.get_radius() + my_ship.get_radius() + 5):
...
def group_collide(group, other_project):
    for sprite in set(group):
        if sprite.collide:
            group.remove(sprite)
